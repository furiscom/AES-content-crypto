...
if (isset($_POST['encrypt_now'])) {
    ...
    // Baca data file yang akan dienkripsi
    $fileData = file_get_contents($file_tmpname);
    $fileData = mysqli_real_escape_string($connect, $fileData);

    // Tentukan direktori penyimpanan file enkripsi
    $uploadDir = 'uploads/'; 
    ...
    $finalfile = uniqid(); 
    $path = $uploadDir . $finalfile . '.rda'; 

    // Simpan file enkripsi
    move_uploaded_file($file_tmpname, $path);


    // Update query INSERT dengan file_data
    $sql = "INSERT INTO file (username, file_name_source, file_name_finish, file_url, file_size, password, tgl_upload, status, keterangan, file) 
    VALUES ('$user', '$final_file', '$finalfile.rda', '$path', '$size2', '$key', now(), '1', '$deskripsi', '$fileData')";

    $query = mysqli_query($connect, $sql);

    if ($query) {
        header('location:enkripsi.php?pesan=berhasil');
    } else {
        echo "Gagal menyimpan data file: " . mysqli_error($connect);
    }
